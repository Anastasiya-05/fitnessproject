...
import by.minenkova.fitnessproject.service.PersonService;
...
public class PersonController {
    ...
    public ModelAndView editPage(@PathVariable("id") Integer id) throws NoSuchEntityException{
    ...
    public ModelAndView savePerson(Model model, @Valid @ModelAttribute("personForm") NewPersonDto personDto, Errors errors) {
        ...
        if (errors.hasErrors()) {
            ...
            Integer age = (Integer)personDto.getAge();
            String phone = personDto.getPhone();

            Person newPerson = new Person(id, firstName, lastName, street, city, email, age, phone);
    ...
    public ModelAndView deletePerson(@PathVariable("id") Integer id) throws NoSuchEntityException {
        Person person = personService.getById(id).orElseThrow(() -> new NoSuchEntityException("Person not found"));
